...
public class AuthController : ControllerBase
{
    ...
    [HttpPost("register")]
    public async Task<ActionResult<ResponseRepository<string>>> Register(RegisterUserDTO registerUserDTO){
        var user = await _userRepository.RegisterUser(registerUserDTO);
        if(!user.Success) return BadRequest(user);
        return Ok(user);
    }
    [HttpPost("login")]
    public async Task<ActionResult<ResponseRepository<AuthenticatedUserDTO>>> Login(LoginUserDTO loginUserDTO){
        var response = await _userRepository.Login(loginUserDTO);
        if(!response.Success) return Unauthorized(response);
        return Ok(response);
    }
    
    [HttpPost("refresh-token")]
    public async Task<ActionResult<ResponseRepository<AuthenticatedUserDTO>>> RefreshToken([FromBody] string accessToken){
        // var refreshToken = Request.Cookies["refreshToken"];
        var response = await _userRepository.RefreshToken(accessToken);
        if(!response.Success) return Unauthorized(response);
        return Ok(response);
    }
    [Authorize(Roles ="Admin")]
    [HttpPut("{id}/update-user")]
    public async Task<ActionResult<ResponseRepository<GetUserDTO>>> UpdateUser([FromRoute] Guid id, [FromBody] UpdateUserDTO updateUserDTO){
        var response = await _userRepository.UpdateUser(id,updateUserDTO);
        if(!response.Success) return BadRequest(response);
        return Ok(response);
    }
    [HttpPost("activate-email/{activationCode}")]
    public async Task<ActionResult<ResponseRepository<string>>> ActivateEmail([FromRoute] Guid activationCode){
        var response = await _userRepository.ActivateUserEmail(activationCode);
        if(!response.Success){
    ...
    public async Task<ActionResult<ResponseRepository<string>>> UpdatePassword([FromBody] ChangeUserPasswordDTO changeUserPasswordDTO){
        var response = await _userRepository.ChangePassword(changeUserPasswordDTO);
        if(!response.Success) return BadRequest(response);
        return Ok(response);
    }
    [Authorize(Roles ="Admin")]
    [HttpGet("users")]
    public async Task<ActionResult<ResponseRepository<GetUserDTO>>> GetUsers([FromQuery]QueryHelper query){
        var response = await _userRepository.GetUsers(query);
        // if(!response.Success) return BadRequest(response);
        return Ok(response);
    }
    [HttpPost("logout")]
    public async Task<ActionResult<ResponseRepository<string>>> Logout(){
        var response = await _userRepository.Logout();
        if(!response.Success) return BadRequest(response);
        return response;
    }
    [HttpGet("{id}")]
    public async Task<ActionResult<ResponseRepository<GetUserDTO>>> GetUser([FromRoute] Guid id){
        var response = await _userRepository.GetUser(id);
        if(!response.Success) return BadRequest(response);
        return Ok(response);
    }
    [HttpPost("get-name")]
    public async Task<ActionResult<ResponseRepository<User>>> GetName(){
        var userName = await _userRepository.GetName();
        if(!userName.Success) return BadRequest(userName);
        return Ok(userName);
    }
    // public ActionResult<string> GetName([FromBody] string accessToken){
    //     var userName = _userRepository.GetName(accessToken);
    //     return Ok(userName);
    // }
}
...
//                    <table cellpadding=""0"" cellspacing=""0"" width=""100%"" role=""presentation"" style=""mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"">
//                      <tr>
//                       <td align=""center"" style=""padding:0;Margin:0;padding-top:10px;padding-bottom:10px;font-size:0px""><img src=""https://eopifrc.stripocdn.email/content/guids/CABINET_67e080d830d87c17802bd9b4fe1c0912/images/55191618237638326.png"" alt="" width=""100"" style=""display:block;font-size:14px;border:0;outline:none;text-decoration:none""></td>
//                      </tr>
//                      <tr>
//                       <td align=""center"" class=""es-m-txt-c"" style=""padding:0;Margin:0;padding-bottom:10px""><h1 style=""Margin:0;font-family:arial, 'helvetica neue', helvetica, sans-serif;mso-line-height-rule:exactly;letter-spacing:0;font-size:46px;font-style:normal;font-weight:bold;line-height:46px;color:#333333"">Activate your account</h1></td>
//                      </tr>
//                      <tr>
//                       <td align=""center"" class=""es-m-p0r es-m-p0l"" style=""Margin:0;padding-top:5px;padding-right:40px;padding-bottom:5px;padding-left:40px""><p style=""Margin:0;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;letter-spacing:0;color:#333333;font-size:14px"">Clink on link below to activate account and start shopping</p></td>
//                      </tr>
//                      <tr>
...
//                          <tr>
//                           <td align=""center"" valign=""top"" style=""padding:0;Margin:0;padding-right:40px""><img title=""Facebook"" src=""https://eopifrc.stripocdn.email/content/assets/img/social-icons/logo-black/facebook-logo-black.png"" alt=""Fb"" width=""32"" style=""display:block;font-size:14px;border:0;outline:none;text-decoration:none""></td>
//                           <td align=""center"" valign=""top"" style=""padding:0;Margin:0;padding-right:40px""><img title=""X"" src=""https://eopifrc.stripocdn.email/content/assets/img/social-icons/logo-black/x-logo-black.png"" alt=""X"" width=""32"" style=""display:block;font-size:14px;border:0;outline:none;text-decoration:none""></td>
//                           <td align=""center"" valign=""top"" style=""padding:0;Margin:0;padding-right:40px""><img title=""Instagram"" src=""https://eopifrc.stripocdn.email/content/assets/img/social-icons/logo-black/instagram-logo-black.png"" alt=""Inst"" width=""32"" style=""display:block;font-size:14px;border:0;outline:none;text-decoration:none""></td>
//                           <td align=""center"" valign=""top"" style=""padding:0;Margin:0""><img title=""Youtube"" src=""https://eopifrc.stripocdn.email/content/assets/img/social-icons/logo-black/youtube-logo-black.png"" alt=""Yt"" width=""32"" style=""display:block;font-size:14px;border:0;outline:none;text-decoration:none""></td>
//                          </tr>
//                        </table></td>
//                      </tr>
//                      <tr>
